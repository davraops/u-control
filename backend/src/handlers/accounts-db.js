...
async function handlePost(event) {
  ...
  const result = await pool.query(
    `INSERT INTO accounts (id, name, type, bank, account_number, balance, currency, tags, is_active)
...
async function handlePut(event) {
  ...
  const result = await pool.query(
    `UPDATE accounts SET ${updateFields.join(', ')} WHERE id = $${paramCount} AND is_active = true RETURNING *`,
    values
  );
...
async function handleDelete(event) {
  ...
  const result = await pool.query(
    'UPDATE accounts SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *',
    [pathParameters.id]
  );
