...
module.exports.handler = async (event) => {
  console.log('Expenses handler called with method:', event.httpMethod);
  ...
  try {
    ...
    console.error('Error in expenses handler:', error);
    return {
      statusCode: 500,
      headers: corsHeaders,
...
async function handleGet(event) {
  ...
  if (pathParameters && pathParameters.id) {
    ...
    const result = await pool.query(
      'SELECT * FROM expenses WHERE id = $1 AND is_active = true',
      [pathParameters.id]
    );
    ...
    return {
      statusCode: 200,
      headers: corsHeaders,
      body: JSON.stringify({ expense: formatExpense(result.rows[0]) })
    };
  } else {
    // Get all expenses
    const result = await pool.query(
      'SELECT * FROM expenses WHERE is_active = true ORDER BY date DESC'
    );
...
async function handlePost(event) {
  ...
  const result = await pool.query(
    `INSERT INTO expenses (id, amount, currency, description, category, account_id, budget_id, date, tags, is_recurring, recurring_pattern, is_active)
...
async function handlePut(event) {
  ...
  if (body.budgetId !== undefined) {
    updateFields.push(`budget_id = $${paramCount++}`);
    values.push(body.budgetId);
  }
  ...
  const result = await pool.query(
    `UPDATE expenses SET ${updateFields.join(', ')} WHERE id = $${paramCount} AND is_active = true RETURNING *`,
    values
  );
...
async function handleDelete(event) {
  ...
  const result = await pool.query(
    'UPDATE expenses SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *',
    [pathParameters.id]
  );
